...
8. Luxembourgish: If someone writes in Luxembourgish, reply in Luxembourgish.${reviewStr}${bookingEmailStr}`;
  ...
  const texts = {
    ...
    lb: { badge: "🇱🇺 Eng KI. All Sproochen. All Clienten.", h1: ["Iert Geschäft.", "All Sproochen.", "Null Effort."], sub: "KI dee mat Iere Clienten op Lëtzebuergesch, Franséisch, Däitsch an Englesch schwätzt. Fakturen, Offerten an E-Mailen - automatesch.", cta: "Gratis Testen", cta2: "Features gesinn", ft: "Alles wat Iert Geschäft brauch", fs: "Einfach KI-Tools dei Stonne spueren", pt: "Einfach, transparent Präisser", ps: "14 Deeg gratis op allen Pläng. Keng verstopp Käschten.", fin: "Prett fir 15+ Stonne d'Woch ze spueren?", fb: "14 Deeg Gratis Testen" },
  };
    ...
    <div style={{ fontFamily: "var(--font)", background: "var(--bg)", minHeight: "100vh" }}>
      ...
      <section className="fu fu4" style={{ maxWidth: 680, margin: "0 auto 56px", padding: "0 24px" }}>
        <div className="m-2col" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}>
          {[{ n: "5", l: "Languages" }, { n: "24/7", l: "Always On" }, { n: "95%", l: "Satisfaction" }, { n: "30s", l: "Setup" }].map((s, i) => (
      ...
      <section id="features" style={{ maxWidth: 860, margin: "0 auto", padding: "0 24px 64px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 32, flexWrap: "wrap", marginBottom: 48, padding: "20px 24px", background: "white", borderRadius: 14, border: "1px solid var(--border)", boxShadow: "var(--shadow)" }}>
          {[{ n: "5 Languages", d: "FR · DE · EN · LU · PT", icon: "🌍" }, { n: "< 3 seconds", d: "Average response", icon: "⚡" }, { n: "24/7", d: "Always available", icon: "🕐" }, { n: "14-day trial", d: "No credit card risk", icon: "🛡️" }].map((s, i) => (
    ...
    <div style={{ fontFamily: "var(--font)", background: "var(--bg)", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ maxWidth: 400, width: "100%" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }} className="fu">
          ...
          <h1 style={{ fontFamily: "var(--display)", fontSize: 28, color: "var(--navy)", marginBottom: 6 }}>Welcome back</h1>
          <p style={{ color: "var(--muted)", fontSize: 14 }}>Sign in to your LuxReplier account</p>
        </div>
        <div className="card fu fu1" style={{ padding: 28 }}>
          {authError && <div style={{ background: "var(--red-soft)", color: "var(--red)", padding: "10px 14px", borderRadius: 8, fontSize: 13, marginBottom: 16, fontWeight: 600 }}>{authError}</div>}
          <div style={{ marginBottom: 14 }}><label>📧 Email Address</label><input type="email" placeholder="your@email.com" value={loginForm.email} onChange={e => setLoginForm({ ...loginForm, email: e.target.value })} onKeyDown={e => e.key === "Enter" && doLogin()} /></div>
          <div style={{ marginBottom: 20 }}><label>🔒 Password</label><input type="password" placeholder="Your password" value={loginForm.password} onChange={e => setLoginForm({ ...loginForm, password: e.target.value })} onKeyDown={e => e.key === "Enter" && doLogin()} /></div>
          <button className="btn btn-p" style={{ width: "100%", padding: 14, fontSize: 15 }} disabled={!loginForm.email || !loginForm.password} onClick={doLogin}>Sign In →</button>
        </div>
        <div style={{ textAlign: "center", marginTop: 16, fontSize: 12, color: "var(--muted)" }}>
          Don't have an account? <span style={{ color: "var(--accent)", cursor: "pointer", fontWeight: 600 }} onClick={() => { setAuthError(""); setView("signup"); }}>Sign up</span>
        </div>
        <div style={{ textAlign: "center", marginTop: 8 }}>
          <button className="btn btn-g" onClick={() => { setAuthError(""); setView("home"); }} style={{ fontSize: 13 }}>← Back to website</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ fontFamily: "var(--font)", background: "var(--bg)", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ maxWidth: 420, width: "100%" }}>
        ...
        <div className="card fu fu1" style={{ padding: 28 }}>
          ...
          {step === 1 && (
            <div>
              {authError && <div style={{ background: "var(--red-soft)", color: "var(--red)", padding: "10px 14px", borderRadius: 8, fontSize: 13, marginBottom: 14, fontWeight: 600 }}>{authError}</div>}
              ...
              <div style={{ marginBottom: 14 }}><label>🔒 Password</label><input type="password" placeholder="At least 8 characters" value={signup.password} onChange={e => setSignup({ ...signup, password: e.target.value })} /></div>
              <div style={{ marginBottom: 20 }}><label>🔑 Access Code (optional)</label><input placeholder="Have an access code? Enter it here" value={signup.ownerCode || ""} onChange={e => setSignup({ ...signup, ownerCode: e.target.value })} /></div>
              <button className="btn btn-p" style={{ width: "100%", padding: 14, fontSize: 15 }} disabled={!signup.name || !signup.email || signup.password.length < 8} onClick={() => { if (signup.ownerCode === "LUXOWNER2026") { setUserPlan("premium"); setView("setup"); } else { doSignup(); } }}>Create Account →</button>
            </div>
          )}
          {step === 2 && (
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>✅</div>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: "var(--navy)", marginBottom: 8 }}>Account created!</h3>
              <p style={{ fontSize: 13, color: "var(--muted)", marginBottom: 20 }}>Your account is ready. Now choose a plan to activate all features.</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
                <button className="btn btn-o" style={{ width: "100%", padding: 12 }} onClick={() => { window.open(STRIPE_LINKS.starter, "_blank"); }}>Starter - €99/mo</button>
                <button className="btn btn-p" style={{ width: "100%", padding: 12 }} onClick={() => { window.open(STRIPE_LINKS.business, "_blank"); }}>Business - €199/mo ⭐ Popular</button>
                <button className="btn btn-o" style={{ width: "100%", padding: 12 }} onClick={() => { window.open(STRIPE_LINKS.premium, "_blank"); }}>Premium - €299/mo</button>
              </div>
              <button className="btn btn-g" style={{ width: "100%", fontSize: 13 }} onClick={() => setView("setup")}>Skip for now - explore first →</button>
            </div>
          )}
          <div style={{ marginTop: 18, textAlign: "center", fontSize: 11, color: "var(--muted)" }}>🔒 Secured by Stripe · 256-bit encryption</div>
        </div>
        <div style={{ textAlign: "center", marginTop: 16, fontSize: 12, color: "var(--muted)" }}>Already have an account? <span style={{ color: "var(--accent)", cursor: "pointer", fontWeight: 600 }} onClick={() => { setAuthError(""); setView("login"); }}>Log in</span></div>
