...
export default function LuxReplier() {
  ...
  // QR code generation for Install tab (must be after widgetId declaration)
  useEffect(() => {
    if (section !== "widget" || !widgetId) return;
    const chatUrl = `https://chat.luxreplier.lu/${widgetId}`;
    const genQR = () => {
      if (window.QRCode) {
        window.QRCode.toDataURL(chatUrl, { width: 240, margin: 2, color: { dark: "#1A2744", light: "#FFFFFF" } })
          .then(url => setQrDataUrl(url)).catch(() => {});
      }
    };
    if (document.getElementById("qrcode-cdn")) { genQR(); return; }
                  ...
                  <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      <button className="btn" style={{ padding: "10px 20px", fontSize: 13, borderRadius: 10, background: "rgba(124,58,237,0.15)", color: "#7C3AED", border: "none", fontWeight: 600 }}
                        onClick={() => { const a = document.createElement("a"); a.href = qrDataUrl; a.download = `${setup.bizName || "luxreplier"}-qr-code.png`; a.click(); }}>
