...
.dash-dark .dcard-static,.dash-dark .dcard-static:hover{transform:none!important;transition:none!important}
...
export default function LuxReplier() {
  ...
  const [showPw, setShowPw] = useState({ login: false, signup: false, signupConfirm: false, reset: false, resetConfirm: false, settings: false, settingsConfirm: false });
      ...
      if (data.id) {
        ...
        return fetch(`${SUPABASE_URL}/rest/v1/businesses?user_id=eq.${data.id}&select=*`, {
          ...
          if (rows && rows[0]) {
            ...
            // Check if wizard was completed — if not, resume at saved step
            const savedStep = localStorage.getItem("lx_wizard_step");
            if (savedStep && savedStep !== "done") {
              setWizardStep(parseInt(savedStep) || 1);
              setView("setup");
            } else {
              setView("app");
            }
...
${(() => { const fields = setup.industryFields || {}; const entries = Object.entries(fields).filter(([,v]) => v && v.trim()); if (entries.length === 0) return ""; const fieldLabels = {}; (selectedType.extraFields || []).forEach(f => { fieldLabels[f.key] = f.label; }); return "Industry-specific details:\n" + entries.map(([k,v]) => `- ${fieldLabels[k] || k}: ${v}`).join("\n"); })()}
</business_info>
...
${bookingEmailStr}`;
        ...
        <div style={{ maxWidth: 400, width: "100%" }}>
          ...
          <div className="card fu fu1" style={{ padding: 28, marginBottom: 16 }}>
            ...
            <div style={{ marginBottom: 8 }}>
              ...
              <div style={{ position: "relative" }}>
              <input type={showPw.login ? "text" : "password"} placeholder="Enter your password" value={loginForm.password}
                onChange={e => setLoginForm({ ...loginForm, password: e.target.value })}
                onKeyDown={e => e.key === "Enter" && doLogin()}
                style={{ fontSize: 15, paddingRight: 40 }} />
              <button type="button" tabIndex={-1} onClick={() => setShowPw(p => ({ ...p, login: !p.login }))} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 4, color: "var(--muted)", display: "flex" }}>
            ...
            <div style={{ textAlign: "right", marginBottom: showForgot ? 12 : 24 }}>
              <span style={{ fontSize: 12, color: "var(--accent)", cursor: "pointer", fontWeight: 600 }}
                onClick={() => { setShowForgot(!showForgot); setAuthError(""); }}>
                {showForgot ? "← Back to login" : "Forgot password?"}
              </span>
            </div>
            {showForgot && (
              <div style={{ marginBottom: 24, padding: 16, background: "var(--accent-soft)", borderRadius: 10, border: "1px solid var(--accent)" }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "var(--navy)", marginBottom: 10 }}>Reset your password</div>
                <input type="email" placeholder="Enter your email address" value={forgotEmail}
                  onChange={e => setForgotEmail(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && forgotEmail && (async () => {
                    setAuthError("");
                    try {
                      ...
                      if (res.ok) { setAuthError(""); setShowForgot(false); setForgotEmail(""); alert("Password reset email sent — check your inbox."); }
                      else { const d = await res.json(); setAuthError(d.error_description || d.msg || "Failed to send reset email."); }
                    } catch { setAuthError("Connection error. Please try again."); }
                  })()}
                  style={{ fontSize: 14, marginBottom: 10 }} />
                <button className="btn btn-p" style={{ width: "100%", padding: "10px", fontSize: 13, borderRadius: 10 }}
  ...
  if (view === "resetPassword") return (
    <div style={{ fontFamily: "var(--font)", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", padding: "40px 24px" }}>
      <style>{css}</style>
      <div style={{ maxWidth: 420, width: "100%" }}>
        ...
        <div className="fu" style={{ marginBottom: 24 }}>
          <h1 style={{ fontFamily: "var(--display)", fontSize: 28, fontWeight: 800, color: "var(--navy)", marginBottom: 8, textAlign: "center" }}>Set Your New Password</h1>
          <p style={{ color: "var(--muted)", fontSize: 14, textAlign: "center" }}>Enter your new password below.</p>
        </div>
        {resetMsg && (
          <div className="fu" style={{ background: resetMsg.includes("updated") ? "var(--green-soft)" : "var(--red-soft)", border: resetMsg.includes("updated") ? "1px solid var(--green)" : "1px solid #FCA5A5", borderRadius: 10, padding: "12px 16px", marginBottom: 20, textAlign: "center" }}>
            <span style={{ fontSize: 13, color: resetMsg.includes("updated") ? "var(--green)" : "var(--red)", fontWeight: 600 }}>{resetMsg}</span>
          </div>
        )}
        <div className="card fu fu1" style={{ padding: 28 }}>
          <div style={{ marginBottom: 18 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", display: "block", marginBottom: 6 }}>New Password</label>
            <div style={{ position: "relative" }}>
            <input type={showPw.reset ? "text" : "password"} placeholder="At least 8 characters" value={resetNewPass}
              onChange={e => setResetNewPass(e.target.value)}
              style={{ fontSize: 15, paddingRight: 40 }} />
            <button type="button" tabIndex={-1} onClick={() => setShowPw(p => ({ ...p, reset: !p.reset }))} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 4, color: "var(--muted)", display: "flex" }}>
              {showPw.reset ? <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg> : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>}
            </button>
            </div>
          </div>
          <div style={{ marginBottom: 24 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", display: "block", marginBottom: 6 }}>Confirm New Password</label>
            <div style={{ position: "relative" }}>
            <input type={showPw.resetConfirm ? "text" : "password"} placeholder="Re-enter your new password" value={resetConfirmPass}
              onChange={e => setResetConfirmPass(e.target.value)}
              onKeyDown={e => e.key === "Enter" && resetNewPass.length >= 8 && resetNewPass === resetConfirmPass && document.getElementById("reset-btn")?.click()}
              style={{ fontSize: 15, paddingRight: 40 }} />
            <button type="button" tabIndex={-1} onClick={() => setShowPw(p => ({ ...p, resetConfirm: !p.resetConfirm }))} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 4, color: "var(--muted)", display: "flex" }}>
              {showPw.resetConfirm ? <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg> : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>}
            </button>
            </div>
            {resetConfirmPass.length > 0 && resetConfirmPass !== resetNewPass && (
              <div style={{ fontSize: 12, color: "var(--red)", fontWeight: 600, marginTop: 4 }}>Passwords do not match</div>
            )}
          </div>
          <button id="reset-btn" className="btn btn-p" style={{ width: "100%", padding: "14px", fontSize: 15, fontWeight: 700, borderRadius: 12 }}
            disabled={resetNewPass.length < 8 || resetNewPass !== resetConfirmPass}
            onClick={async () => {
              setResetMsg("");
              try {
                ...
                if (res.ok) {
                  ...
                  localStorage.removeItem("lx_token");
                  setTimeout(() => { setView("login"); setResetNewPass(""); setResetConfirmPass(""); setResetMsg(""); setResetToken(null); }, 2000);
                } else {
                  const d = await res.json();
                  setResetMsg(d.error_description || d.msg || "Failed to update password. The link may have expired.");
                }
              } catch { setResetMsg("Connection error. Please try again."); }
            }}>
            Save New Password →
          </button>
        </div>
        <div style={{ textAlign: "center", marginTop: 16 }}>
          <button className="btn btn-g" onClick={() => { setView("login"); setResetToken(null); setResetNewPass(""); setResetConfirmPass(""); setResetMsg(""); }} style={{ fontSize: 13, color: "var(--muted)" }}>← Back to login</button>
        </div>
      </div>
    </div>
  );
            ...
            <div className="fu">
              ...
              <div className="card" style={{ padding: 28, marginBottom: 16 }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", display: "block", marginBottom: 6 }}>Password</label>
                  <div style={{ position: "relative" }}>
                  <input type={showPw.signup ? "text" : "password"} placeholder="At least 8 characters" value={signup.password} onChange={e => setSignup({ ...signup, password: e.target.value })} style={{ fontSize: 15, paddingRight: 40 }} />
                  <button type="button" tabIndex={-1} onClick={() => setShowPw(p => ({ ...p, signup: !p.signup }))} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 4, color: "var(--muted)", display: "flex" }}>
                    {showPw.signup ? <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg> : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>}
                  </button>
                  </div>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", display: "block", marginBottom: 6 }}>Confirm Password</label>
                  <div style={{ position: "relative" }}>
                  <input type={showPw.signupConfirm ? "text" : "password"} placeholder="Re-enter your password" value={confirmPass} onChange={e => setConfirmPass(e.target.value)} style={{ fontSize: 15, paddingRight: 40 }} />
                  <button type="button" tabIndex={-1} onClick={() => setShowPw(p => ({ ...p, signupConfirm: !p.signupConfirm }))} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 4, color: "var(--muted)", display: "flex" }}>
                    {showPw.signupConfirm ? <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg> : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>}
                  </button>
                  </div>
                  {confirmPass.length > 0 && confirmPass !== signup.password && (
                    <div style={{ fontSize: 12, color: "var(--red)", fontWeight: 600, marginTop: 4 }}>Passwords do not match</div>
                  )}
                </div>
                <div style={{ marginBottom: 24 }}>
                ...
                <button className="btn btn-p" style={{ width: "100%", padding: "15px", fontSize: 15, fontWeight: 700, borderRadius: 12 }}
                  disabled={!signup.name || !signup.email || signup.password.length < 8 || signup.password !== confirmPass}
  ...
  if (view === "setup") {
    ...
    const handleActivate = () => {
      ...
      saveBusinessData();
      try { localStorage.setItem("lx_wizard_step", "done"); } catch {}
      setView("app");
    };
        ...
        <div style={{ background: "white", borderBottom: "1px solid var(--border)", padding: "12px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <button className="btn btn-g" style={{ fontSize: 12 }} onClick={() => {
              saveBusinessData();
              try { localStorage.setItem("lx_wizard_step", String(wizardStep)); } catch {}
              if (msgs.length === 0) {
            ...
            {wizardStep === 5 && (
              <div className="fu fu1">
                ...
                {selectedType.extraFields && selectedType.extraFields.length > 0 && (() => {
                  const fields = selectedType.extraFields;
                  const filledCount = fields.filter(f => (setup.industryFields || {})[f.key]?.trim()).length;
                  const fieldIcons = { cuisine: "🍳", avgPrice: "💰", capacity: "🪑", dietary: "🥗", reservation: "📅", parking: "🅿️", areas: "📍", propTypes: "🏢", priceRange: "💶", viewingProcess: "👁️", commission: "📊", docsRequired: "📋", services: "🛠️", brands: "✨", booking: "📅", specialties: "⭐", treatments: "💊", insurance: "🏥", newPatients: "👤", emergency: "🚨", waitTime: "⏳", clientTypes: "👥", pricing: "💰", consultation: "🤝", languages: "🌍", products: "📦", delivery: "🚚", returns: "↩️", area: "📍", responseTime: "⚡", certifications: "✅", targetClients: "🎯" };
                  return (
                    <div className="card" style={{ padding: 24, marginBottom: 14, border: "2px solid var(--accent-soft)" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 700, color: filledCount === fields.length ? "var(--green)" : "var(--muted)" }}>
                          {filledCount}/{fields.length} filled
                        </span>
                      </div>
                      <p style={{ fontSize: 12, color: "var(--muted)", marginBottom: 14 }}>These details help your AI give precise, accurate answers specific to YOUR business.</p>
                      {/* Completion bar */}
                      <div style={{ height: 4, background: "var(--border)", borderRadius: 3, marginBottom: 16, overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${(filledCount / fields.length) * 100}%`, background: filledCount === fields.length ? "var(--green)" : "var(--accent)", borderRadius: 3, transition: "width .4s ease" }} />
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: fields.length > 4 ? "1fr 1fr" : "1fr", gap: 12 }}>
                        {fields.map((field, fi) => {
                          const val = (setup.industryFields || {})[field.key] || "";
                          const isFilled = val.trim().length > 0;
                          return (
                            <div key={fi} style={{ position: "relative" }}>
                              <label style={{ fontSize: 12, fontWeight: 600, color: isFilled ? "var(--text)" : "var(--muted)", display: "flex", alignItems: "center", gap: 4, marginBottom: 5 }}>
                                <span style={{ fontSize: 14 }}>{fieldIcons[field.key] || "📝"}</span>
                                {field.label}
                                {isFilled && <span style={{ color: "var(--green)", fontSize: 11 }}>✓</span>}
                              </label>
                              <input placeholder={field.placeholder}
                                value={val}
                                onChange={e => setSetup({ ...setup, industryFields: { ...(setup.industryFields || {}), [field.key]: e.target.value } })}
                                style={{ fontSize: 13, borderColor: isFilled ? "var(--green)" : "var(--border)" }} />
                            </div>
                          );
                        })}
                      </div>
              ...
              {wizardStep < WIZARD_STEPS && (
                <button className="btn btn-p" style={{ flex: 2, padding: 13, fontSize: 14, fontWeight: 700 }}
                  ...
                  onClick={() => {
                    ...
                    saveBusinessData(); // Auto-save at every step
                    try { localStorage.setItem("lx_wizard_step", String(nextStep)); } catch {}
  ...
  return (
    <div className="dash-dark" style={{ fontFamily: "var(--font)", height: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: "#0D1220", borderBottom: "1px solid #2A3A5C", padding: "10px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => setSection("settings")}>
        ...
        <div className="hide-m" style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }} onClick={() => setSection("settings")}>
        ...
        {section === "widget" && (
          <div style={{ flex: 1, overflow: "auto", padding: "24px 18px", background: "#0F1629" }}>
            <div style={{ maxWidth: 620, margin: "0 auto" }}>
              ...
              {/* Method 3: QR Code */}
              <div className="dcard dcard-static" style={{ padding: 22, marginBottom: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
        ...
        {section === "settings" && (
          <div style={{ flex: 1, overflow: "auto", padding: "24px 18px", background: "#0F1629" }}>
            <div style={{ maxWidth: 620, margin: "0 auto" }}>
              ...
              <div style={{ marginBottom: 28 }}>
                <h3 style={{ fontFamily: "var(--display)", fontSize: 22, color: "#F0F4FF", marginBottom: 4, display: "flex", alignItems: "center", gap: 10 }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#2D5BFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
                  Settings
                </h3>
                <p style={{ fontSize: 13, color: "#8B9CC8" }}>Manage your account, subscription and preferences.</p>
              </div>

              {/* Section A — Account */}
              <div className="dcard" style={{ padding: 22, marginBottom: 16 }}>
                <h4 style={{ fontSize: 13, fontWeight: 700, color: "#8B9CC8", textTransform: "uppercase", letterSpacing: ".5px", margin: "0 0 16px" }}>Account</h4>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 14, marginBottom: 12, padding: "10px 14px", background: "#0D1220", borderRadius: 10 }}>
                  <span style={{ color: "#8B9CC8" }}>Email</span>
                  <span style={{ color: "#F0F4FF", fontWeight: 600 }}>{authUser?.email || "Not signed in"}</span>
                </div>
                {!showChangePass ? (
                  <button className="btn" style={{ padding: "10px 16px", fontSize: 13, background: "rgba(45,91,255,0.1)", color: "#2D5BFF", border: "none", borderRadius: 10, fontWeight: 600 }}
                    onClick={() => { setShowChangePass(true); setSettingsMsg(""); }}>
                    Change Password
                  </button>
                ) : (
                  <div style={{ padding: 16, background: "#0D1220", borderRadius: 10, border: "1px solid #2A3A5C" }}>
                    <div style={{ marginBottom: 10 }}>
                      <label style={{ fontSize: 12, fontWeight: 600, color: "#8B9CC8", display: "block", marginBottom: 4 }}>New Password</label>
                      <div style={{ position: "relative" }}>
                      <input type={showPw.settings ? "text" : "password"} placeholder="At least 8 characters" value={newPass} onChange={e => setNewPass(e.target.value)}
                        style={{ background: "#1A2744", border: "1px solid #2A3A5C", color: "#F0F4FF", borderRadius: 8, padding: "10px 40px 10px 12px", width: "100%", fontSize: 13, fontFamily: "var(--font)", outline: "none" }} />
                      <button type="button" tabIndex={-1} onClick={() => setShowPw(p => ({ ...p, settings: !p.settings }))} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 4, color: "#8B9CC8", display: "flex" }}>
                    ...
                    <div style={{ marginBottom: 12 }}>
                      <label style={{ fontSize: 12, fontWeight: 600, color: "#8B9CC8", display: "block", marginBottom: 4 }}>Confirm New Password</label>
                      <div style={{ position: "relative" }}>
                      <input type={showPw.settingsConfirm ? "text" : "password"} placeholder="Re-enter new password" value={confirmNewPass} onChange={e => setConfirmNewPass(e.target.value)}
                        style={{ background: "#1A2744", border: "1px solid #2A3A5C", color: "#F0F4FF", borderRadius: 8, padding: "10px 40px 10px 12px", width: "100%", fontSize: 13, fontFamily: "var(--font)", outline: "none" }} />
                      <button type="button" tabIndex={-1} onClick={() => setShowPw(p => ({ ...p, settingsConfirm: !p.settingsConfirm }))} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 4, color: "#8B9CC8", display: "flex" }}>
                        {showPw.settingsConfirm ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg> : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>}
                      </button>
                      </div>
                      {confirmNewPass.length > 0 && confirmNewPass !== newPass && (
                        <div style={{ fontSize: 12, color: "#FF6B6B", fontWeight: 600, marginTop: 4 }}>Passwords do not match</div>
                      )}
                    </div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button className="btn btn-p" style={{ padding: "8px 16px", fontSize: 12, borderRadius: 8 }}
                        disabled={newPass.length < 8 || newPass !== confirmNewPass}
                        onClick={async () => {
                          try {
                            const token = localStorage.getItem("lx_token");
                            const res = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
                              method: "PUT", headers: { ...sbHeaders(token), "Content-Type": "application/json" },
                              body: JSON.stringify({ password: newPass })
                            });
                            if (res.ok) { setSettingsMsg("Password updated successfully."); setShowChangePass(false); setNewPass(""); setConfirmNewPass(""); }
                            else { const d = await res.json(); setSettingsMsg(d.error_description || "Failed to update password."); }
                          } catch { setSettingsMsg("Connection error."); }
                        }}>
                        Save Password
                      </button>
                      <button className="btn" style={{ padding: "8px 16px", fontSize: 12, background: "transparent", color: "#8B9CC8", border: "none" }}
                        onClick={() => { setShowChangePass(false); setNewPass(""); setConfirmNewPass(""); }}>
                        Cancel
                      </button>
                    </div>
                  </div>
                )}
                {settingsMsg && <div style={{ marginTop: 10, fontSize: 13, color: settingsMsg.includes("success") ? "#00C896" : "#FF6B6B", fontWeight: 600 }}>{settingsMsg}</div>}
              </div>

              {/* Section B — Subscription */}
              <div className="dcard" style={{ padding: 22, marginBottom: 16 }}>
                <h4 style={{ fontSize: 13, fontWeight: 700, color: "#8B9CC8", textTransform: "uppercase", letterSpacing: ".5px", margin: "0 0 16px" }}>Subscription</h4>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 14, marginBottom: 10, padding: "10px 14px", background: "#0D1220", borderRadius: 10 }}>
                  <span style={{ color: "#8B9CC8" }}>Current Plan</span>
                  <span className="dtag" style={{ background: "rgba(45,91,255,0.15)", color: "#2D5BFF" }}>{plan.label}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 14, marginBottom: 16, padding: "10px 14px", background: "#0D1220", borderRadius: 10 }}>
                  <span style={{ color: "#8B9CC8" }}>Status</span>
                  <span style={{ color: getTrialDaysLeft() <= 3 ? "#FF6B6B" : "#00C896", fontWeight: 600 }}>
                    {getTrialDaysLeft() > 0 ? `${getTrialDaysLeft()} days trial remaining` : "Active subscription"}
                  </span>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  {/* TODO: replace with your Stripe Customer Portal URL from stripe.com/settings/billing/portal */}
                  <button className="btn" style={{ flex: 1, padding: "10px", fontSize: 13, background: "rgba(45,91,255,0.1)", color: "#2D5BFF", border: "none", borderRadius: 10, fontWeight: 600 }}
                    onClick={() => window.open(STRIPE_LINKS.business, "_blank")}>
                    Manage Billing →
                  </button>
                  {userPlan !== "premium" && (
                    <button className="btn btn-p" style={{ flex: 1, padding: "10px", fontSize: 13, borderRadius: 10 }}
              ...
              <div className="dcard" style={{ padding: 22, border: "1px solid rgba(255,107,107,0.2)" }}>
                <h4 style={{ fontSize: 13, fontWeight: 700, color: "#FF6B6B", textTransform: "uppercase", letterSpacing: ".5px", margin: "0 0 16px" }}>Danger Zone</h4>
                <div style={{ display: "flex", gap: 10 }}>
                  <button className="btn" style={{ flex: 1, padding: "10px", fontSize: 13, background: "rgba(255,107,107,0.1)", color: "#FF6B6B", border: "none", borderRadius: 10, fontWeight: 600 }}
                    onClick={() => { if (window.confirm("Sign out of LuxReplier?")) doLogout(); }}>
                    Sign Out
                  </button>
                  <button className="btn" style={{ flex: 1, padding: "10px", fontSize: 13, background: "rgba(255,50,50,0.1)", color: "#FF4444", border: "none", borderRadius: 10, fontWeight: 600 }}
                    onClick={async () => {
                      if (!window.confirm("This permanently deletes your account and all data. This cannot be undone. Are you sure?")) return;
                      try {
                        const token = localStorage.getItem("lx_token");
                        if (authUser?.id) {
                          await fetch(`${SUPABASE_URL}/rest/v1/businesses?user_id=eq.${authUser.id}`, {
                            method: "DELETE", headers: { ...sbHeaders(token), Prefer: "return=minimal" }
                          });
                        }
                        doLogout();
                      } catch { doLogout(); }
