            ...
            {wizardStep === 5 && (
              <div className="fu fu1">
                <div className="card" style={{ padding: 24, marginBottom: 14 }}>
                  ...
                  <textarea placeholder="Describe your business here - menu, specialities, atmosphere, team, what makes you unique..." rows={4}
                    value={setup.description} onChange={e => setSetup({ ...setup, description: e.target.value })}
                    style={{ minHeight: 100, fontSize: 13 }} />
                </div>
                <div className="card" style={{ padding: 24 }}>
                  <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--navy)", marginBottom: 4 }}>❓ Top 3 Questions Customers Ask</h3>
                  <p style={{ fontSize: 12, color: "var(--muted)", marginBottom: 14 }}>Your AI will answer these perfectly every time. You can add more later.</p>
                  {setup.faqItems.slice(0, 3).map((faq, i) => (
                    <div key={i} style={{ marginBottom: 14, padding: 12, background: "var(--bg)", borderRadius: 10, border: "1px solid var(--border)" }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", marginBottom: 6 }}>Question {i + 1}</div>
                      <input placeholder={["e.g. What are your prices?", "e.g. Do you have parking?", "e.g. Do you take reservations?"][i]}
                        value={faq.q} onChange={e => { const f = [...setup.faqItems]; f[i] = { ...f[i], q: e.target.value }; setSetup({ ...setup, faqItems: f }); }}
                        style={{ marginBottom: 6, fontSize: 13 }} />
                      <textarea placeholder={["e.g. Our main dishes start from €12...", "e.g. Free parking behind our building...", "e.g. Yes! Book via phone or our website..."][i]}
                        value={faq.a} rows={2}
