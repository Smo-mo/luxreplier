...
export default function LuxReplier() {
  ...
  // QR code generation via qrserver.com — free, reliable, no library needed
  useEffect(() => {
    if (section !== "widget" || !widgetId) return;
    const chatUrl = `https://chat.luxreplier.lu/${widgetId}`;
    setQrDataUrl(`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(chatUrl)}`);
  }, [section, widgetId]);
                  ...
                  <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      <button className="btn" style={{ padding: "10px 20px", fontSize: 13, borderRadius: 10, background: "rgba(124,58,237,0.15)", color: "#7C3AED", border: "none", fontWeight: 600 }}
                        onClick={async () => { try { const r = await fetch(qrDataUrl); const b = await r.blob(); const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = `${widgetId}-qr-code.png`; a.click(); } catch { window.open(qrDataUrl, "_blank"); } }}>
