    ...
    <div style={{ fontFamily: "var(--font)", background: "var(--bg)", minHeight: "100vh" }}>
      ...
      <section style={{ maxWidth: 860, margin: "0 auto", padding: "0 24px 64px" }}>
        ...
        <div className="m-stack" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
          {[
            ...
            { icon: "🇪🇺", title: "GDPR compliant, EU hosted", desc: "All data stays in Europe. Fully compliant with Luxembourg and EU data protection law. No data shared with third parties outside the EU.", tag: "Legally safe" },
      ...
      <section style={{ maxWidth: 680, margin: "0 auto", padding: "0 24px 56px" }}>
        ...
        {[
          ...
          { q: "Is my customer data safe and GDPR compliant?", a: "Yes. All data is stored in the EU and LuxReplier is fully GDPR compliant. We never sell your data or use it to train AI models. Read our Privacy Policy for full details." },
        ].map((faq, i) => (
        ...
        <div className="card fu fu1" style={{ padding: 28 }}>
          ...
          {step === 1 && (
            <div>
              ...
              <div style={{ marginBottom: 20 }}><label>🔑 Access Code</label><input placeholder="Have an access code? Enter it here" value={signup.ownerCode || ""} onChange={e => setSignup({ ...signup, ownerCode: e.target.value })} /></div>
              <button className="btn btn-p" style={{ width: "100%", padding: 14, fontSize: 15 }} disabled={!signup.name || !signup.email || signup.password.length < 8} onClick={() => { if (signup.ownerCode === "LUXOWNER2026") { setUserPlan("premium"); setView("setup"); } else { setStep(2); } }}>Continue →</button>
            </div>
          )}
          {step === 2 && (
            <div>
